...
object MyESUtil {
  ...
  def build():Unit = {
    ...
    factory.setHttpClientConfig(new HttpClientConfig.Builder("http://192.168.1.221:9200")
      .multiThreaded(true)
  ...
  /**
    ...
    * @param indexName
    */
  def bulkInsert0(infoList: List[(String,Any)], indexName: String): Unit = {
  ...
  def bulkInsert(infoList: List[(String,Any)], indexName: String): Unit = {
    ...
    if (infoList != null && infoList.size != 0) {
      ...
      for ((id,dauInfo) <- infoList) {
        ...
        val index: Index = new Index.Builder(dauInfo)
          .index(indexName)
            .id(id) //指定文档id，保证幂等性，重复插入相同id文档会做覆盖
          .`type`("_doc")
          .build()
