...
import com.gmall.realtime.bean.{OrderInfo, ProvinceInfo, UserInfo, UserStatus}
import com.gmall.realtime.util._
...
/**
  ...
  * @Author: bizh
  * @Date: 2021/5/8 9:55
  * @Description: 从kafka中读取订单数据，并进行处理
  */
object OrderInfoApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 对DS的结构进行转换 ConsumerRecord[String, String] => value:JsonStr => OrderInfo
...
//    orderInfoDStream.print(1000)
    ...
    /*val orderInfoWithFirstFlagDStream: DStream[OrderInfo] = orderInfoDStream.map {
      orderInfo => {
        ...
        var sql: String = s"select user_id, if_consumed from user_status0523 where user_id='${user_id}'"
        val userStatusList: List[JSONObject] = PhoenixUtil.queryList(sql)
        if (userStatusList != null && userStatusList.size > 0) {
          orderInfo.if_first_order = "0" // 非首单
        } else {
          orderInfo.if_first_order = "1" // 首单
        }
    ...
    val orderInfoWithFirstFlagDStream: DStream[OrderInfo] = orderInfoDStream.mapPartitions {
      orderInfoItr => {
        ...
        //根据用户集合到Phoenix中查询 看看哪些用户下过单 坑1
...
//    orderInfoWithFirstFlagDStream.print(1000)
    ...
    val orderInfoWithFirstFlagFixDStream: DStream[OrderInfo] = groupByKeyDStream.flatMap {
      case (userId, orderInfoItr) => {
        val orderInfoList: List[OrderInfo] = orderInfoItr.toList
        // 判断在一个采集周期内，用户是否下了多个订单
        if (orderInfoList != null && orderInfoList.size > 1) {
          // 如果下了多个订单，则按照下单时间升序排序
    ...
    //=================5.和省份维度表进行关联===========================================
    // 5.1 方案1：以分区为单位，对订单数据进行处理，和Phoenix的订单表进行关联
    /*val orderInfoWithProvinceDStream: DStream[OrderInfo] = orderInfoWithFirstFlagFixDStream.mapPartitions {
      orderInfoItr => {
        // 转为List
        val orderInfoList: List[OrderInfo] = orderInfoItr.toList
        // 获取当前分区中订单对应的省份id
        ...
        val sql: String = s"select id,name,area_code,iso_code from gmall0523_province_info where id in('${provinceIdList.mkString("','")}')"
    ...
    val orderInfoWithProvinceDStream: DStream[OrderInfo] = orderInfoWithFirstFlagFixDStream.transform {
      rdd => {
        // 此处代码在driver端执行,从Phoenix查询所有省份数据
...
//    orderInfoWithProvinceDStream.print(1000)
    ...
    //=================6.和用户维度表进行关联===========================================
    // 以分区为单位对数据进行处理，每一个分区拼接一个sql，到Phoenix上查询用户数据
...
//    orderInfoWithUserInfoDStream.print(1000)
    ...
    //=================3.维护首单用户状态 || 保存订单到ES中===========================================
    // 如果当前用户为首单用户， 在进行首单标记之后， 应该将用户的消费状态保存到hbase中，等下次用户再次下单时，就不是首单
    ...
    orderInfoWithUserInfoDStream.foreachRDD{
      rdd => {
        ...
        // 注意：在使用saveToPhoenix方法时，要求RDD中存放数据的属性个数和Phoenix表中字段数必须一致
